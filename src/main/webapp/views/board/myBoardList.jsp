					...
					<div class="cards-wrap">
						...
						<%for(Board b :list){ %>
							...
							<ul class="paging-area" align="center">
								<%if(currentPage != 1){ %>
									<li><button onclick="location.href='<%=contextPath%>/detail.bo?cpage=<%=currentPage - 1%>'">[이전]</button></li>
								<% }%>
								...
								<%for(int p = startPage; p<=endPage; p++) {%>
									...
									<%}else{ %>
										<li><button onclick="location.href='<%=contextPath%>/detail.bo?cpage=<%=p%>'"><%=p %></button></li>
									   <%} %>
								   <%} %>
								   
								   <%if(currentPage != maxPage){ %>
									<li><button onclick="location.href='<%=contextPath%>/detail.bo?cpage=<%=currentPage + 1%>'">[다음]</button></li>
								   <% }%>
							</ul>   
                </div>  
